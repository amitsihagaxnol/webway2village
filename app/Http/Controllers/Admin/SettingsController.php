...
class SettingsController extends Controller
{
        ...
        if (!$request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                ...
                if (env('APP_MODE', '') != 'test') {
                    ...
                    $request['recaptcha_preference'] = (isset($request->recaptcha_preference) && $request->recaptcha_preference == 'disable') ? $request->recaptcha_preference : implode(',', $request->recaptcha_preference);
        ...
        if (! $request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                ...
                if (env('APP_MODE', '') != 'test') {
                    ...
                    changeEnvironmentVariable('GOOGLE_RECAPTCHA_KEY' , $request->google_recaptcha_key);
                    changeEnvironmentVariable('GOOGLE_RECAPTCHA_SECRET' , $request->google_recaptcha_secret);
  ...
  public function defaultImage(Request $request)
    {
            $defaultImage = DefaultImage::first();
            if ($request->hasFile('property_default_pic')) {
                $file = $request->file('property_default_pic');

                // Validate file
                $validExtensions = ['png', 'jpg', 'jpeg', 'gif', 'ico'];
                $extension = $file->getClientOriginalExtension();
                if (!in_array($extension, $validExtensions)) {
                    return back()->withErrors(['error' => 'Invalid file format. Allowed formats are PNG, JPG, JPEG, GIF, ICO.']);
                }
                ...
                $filePath = 'public/front/defaultProperty/images/';
                if ($file->move($filePath, $fileName)) {
                    // Delete all records of the model
                    DefaultImage::truncate();

                    // Insert the new image into the database
                    DefaultImage::create(['property_default_pic' => $fileName]);

                    Common::one_time_message('success', 'Image uploaded successfully.');
                   return redirect('admin/default-image');
                } else {
                    Common::one_time_message('error', 'Failed to upload image.');
                   return redirect('admin/default-image');
                }

            }
            return view('admin.settings.default_image', compact('defaultImage'));
