...
class PropertiesController extends Controller
{
    ...
    public function add(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                ...
                $property->name            = $request->map_address;
    ...
    public function listing(Request $request, CalendarController $calendar)
    {
        ...
        if ($step == 'basics') {
            if ($request->isMethod('post')) {
                ...
                PropertyRoomType::where('property_id', $property_id)->delete();

                if (isset($request->room_type)) {
                    // dd($request->all());
                    $defaultSet = false; // Flag to track if the default has been set
                    foreach ($request->room_type as $key => $roomType) {
                        if (isset($request->room_type[$key])) {
                            ...
                            $propertyRoomType = new PropertyRoomType();
                            ...
                            if (!$defaultSet) {
                                $propertyRoomType->is_default = 1;
                                $defaultSet = true;
                            } else {
                                $propertyRoomType->is_default = 0;
                            }
            ...
            $data['room_type']      = RoomType::where('status', 'Active')->get();
            $data['room_type_table_data']  = PropertyRoomType::where('property_id', $property_id)->get();
        ...
        elseif ($step == 'products') {
             ...
             if ($request->isMethod('post')) {
                ...
                return redirect('admin/listing/'.$property_id.'/type_of_organic_products');
             }
        ...
        elseif ($step == 'type_of_organic_products') {
             ...
             if ($request->isMethod('post')) {
                ...
                return redirect('admin/listing/'.$property_id.'/sustainable_farm_practices');
             }
         ...
         elseif ($step == 'sustainable_farm_practices') {
             ...
             if ($request->isMethod('post')) {
                  ...
                  return redirect('admin/listing/'.$property_id.'/irrigation_method_farm_practices');
             }
         ...
         elseif ($step == 'irrigation_method_farm_practices') {
             ...
             if ($request->isMethod('post')) {
                  ...
                  return redirect('admin/listing/'.$property_id.'/soil_health_and_fertility_practices');
             }
          ...
          elseif ($step == 'soil_health_and_fertility_practices') {
             ...
             if ($request->isMethod('post')) {
                   ...
                   return redirect('admin/listing/'.$property_id.'/pest_and_disease_farm_practices');
             }
           ...
           elseif ($step == 'pest_and_disease_farm_practices') {
             ...
             if ($request->isMethod('post')) {
                    ...
                    return redirect('admin/listing/'.$property_id.'/location_near_me');
             }
       ...
       elseif ($step == 'location_near_me') {
             ...
             if ($request->isMethod('post')) {
                   ...
                   return redirect('admin/listing/'.$property_id.'/facilities');
             }
         ...
         elseif ($step == 'facilities') {
            ...
            $data['facilityAll'] = Facility::latest()->where('status', 1)->get();

            if ($request->isMethod('post')) {
                $checkfacility = FarmHouseFacility::where('property_id', $property_id)->get();
                if ($checkfacility->isNotEmpty()) {
                    FarmHouseFacility::where('property_id', $property_id)->delete();
                }

                $facility_ids = $request->input('facility_id');
                $facility_codes = $request->input('facility_code');
                $descriptions = $request->input('description');
                $prices = $request->input('price');

                // Loop through the facility details arrays
                foreach ($facility_ids as $key => $facility_id) {
                    // Fetch the facility name using the ID
                    $facilityName = Facility::where('id', $facility_id)->value('name');

                    // Create a new FarmHouseFacility record for each facility
                    FarmHouseFacility::create([
                        'property_id' => $property_id,
                        'name' => $facilityName,
                        'facility_id' => $facility_id,
                        'facility_code' => $facility_codes[$key],
             ...
             if ($request->isMethod('post')) {
                  ...
                  $checkActivity =  FarmHouseActivity::where('property_id',$property_id)->get();
                    ...
                    $activity_description = $request->input('description');
                     ...
                     return redirect('admin/listing/'.$property_id.'/general_informations');
             }
    ...
    public function featuredStatusUpdate(Request $request){
            ...
            Properties::where('id',$id)->update(['featured_status'=>$status]);
            return "success";

    }

    public function setPropertyRoomDefault($id) {
        PropertyRoomType::find($id)->update(['is_default' => 1]);
        $propertyId = PropertyRoomType::find($id)->property_id;
        PropertyRoomType::where('property_id', $propertyId)->where('id', '<>', $id)->update(['is_default' => 0]);

        Common::one_time_message('success', 'Room Type Default Success!');
