...
class FarmHouseOwnerDataTable extends DataTable
{
    public function ajax()
    {
        return datatables()
            ...
            $farmHouse = '<form action="' . url('admin/properties') . '" method="GET">
                         <input type="hidden" value="' . $users->id . '" name="user_id">
                        <button type="submit" class="btn btn-xs btn-success">Farm House</button>
                        </form>&nbsp;';
            $bookings = '<form action="' . url('admin/bookings') . '" method="GET">
                <input type="hidden" value="' . $users->id . '" name="user_id">
                <button type="submit" class="btn btn-xs btn-success">Bookings</button>
                </form>&nbsp;';

                $edit = \Helpers::has_permission(Auth::guard('admin')->user()->id, 'edit_customer') ?'<a href="' . url('admin/edit-farm-house-owner/' . $users->id) . '" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>&nbsp;' : '';

                $delete = \Helpers::has_permission(Auth::guard('admin')->user()->id, 'delete_customer') ?'<a href="' . url('admin/delete-farm-house-owner/' . $users->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="fa fa-trash"></i></a>&nbsp;' : '';

                return $farmHouse. $bookings. $edit . $delete;
            })
