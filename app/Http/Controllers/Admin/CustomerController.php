...
class CustomerController extends Controller
{
    ...
    public function add(Request $request, EmailController $email_controller)
    {
        if (! $request->isMethod('post')) {
            ...
            return view('admin.customers.add')->with(compact('states','districts'));
    ...
    public function fetchStateList(Request $request)
    {
         ...
         $states =  State::where(['country_id'=>$country_id,'status'=>'active'])->get();
         ...
         return $country_id;
          $statefirst =  State::where(['country_id'=>$country_id,'status'=>'active'])->first();
          return $statefirst;
         $districts =  District::where(['state_id'=>$statefirst->id,'status'=>'active'])->get();
         $diststr = '<option value="">select district</option>';
         foreach ($districts as $district) {
                
                $diststr .= '<option value="' . $district->id . '">' . $district->name . '</option>';
         }
            
        return ['states'=>$str,'districts'=>$diststr];
    ...
    public function customerVerification(Request $request,$id){
            ...
            return view('admin.customers.verification', $data);
                 ...
                 return redirect()->to('admin/customer/verification/' . $id);
