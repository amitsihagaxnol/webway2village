    ...
    <section class="content">
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-body">
              <form class="form-horizontal" enctype='multipart/form-data' action="{{ url('admin/vilage-properties') }}" method="GET" accept-charset="UTF-8">
                {{ csrf_field() }}
                        ...
                        <select class="form-control" name="state" id="state">
                            ...
                            @if ($states)
                              ...
                              @foreach($states as $state)
                                <option value="{{ $state->id }}" {{ $state->id == $state_data ? "selected" : '' }}>{{ $state->name }}</option>
                              @endforeach
                        ...
                        <select class="form-control" name="district" id="district">
                            ...
                            @if ($districts)
                              ...
                              @foreach($districts as $district)
                                <option value="{{ $district->id }}" {{ $district->id == $district_data ? "selected" : '' }}>{{ $district->name }}</option>
                              @endforeach
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Farm Houses Management</h3>
              @if (Helpers::has_permission(Auth::guard('admin')->user()->id, 'add_properties'))
                <div class="pull-right"><a class="btn btn-success f-14" href="{{ url('admin/village-add-properties') }}">Add Farm Houses</a></div>
              @endif
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                   <div class="alert alert-success featurestatusmsg" role="alert">
                        Update Feature Status Successfully
                    </div>
