...
@push('meta_data')
<title>{{$refunds->heading}}</title>
 <meta property="og:url"                content="{{ isset($shareLink) ? $shareLink : url('/') }}" />
		<meta property="og:type"               content="article" />
		<meta property="og:title"              content="{{$refunds->meta_title}}" />
		<meta property="og:description"        content="{{$refunds->meta_description}}" />
...
@section('main')
	<input type="hidden" id="front_date_format_type" value="{{ Session::get('front_date_format_type') }}">
        ...
        <!-- Header End -->
        <div class="at-tittel_banner " style="background-image:linear-gradient(0deg, rgba(34,34,34,0.4823179271708683) 0%, rgba(34,34,34,0.4599089635854342) 100%), url('http://web.way2village.in/public/images/cms/banner/{{$refunds->image}}');">
            <div class="container">
                <div class="row">
                    <div class="col-md-12 text-center">
                        <h2 class="text-center">{{$refunds->heading}}</h2>
                    </div>
                </div>
            </div>
        </div>
